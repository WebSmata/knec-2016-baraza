<?php
	function js_signin() {
		...
		$loginkey = md5($_POST['password']);
			...
			$_SESSION['imanidairy_user'] = js_let_me_user($loginname, $loginkey);
			$_SESSION['imanidairy_account'] = js_logged_account($loginname);
	...
	function js_is_logged(){
		$myloginid = isset( $_SESSION['imanidairy_user'] ) ? $_SESSION['imanidairy_user'] : "";
	...
	function js_signin_modal() {
	  if ( isset( $_POST['LetMeIn'] ) ) {
		...
		if( $database->js_num_rows( $js_db_query ) > 0 ) {
		    ...
		    $_SESSION['imanidairy_user'] = $userid;			
...
function logout() {
  unset( $_SESSION['imanidairy_user'] );
  unset( $_SESSION['imanidairy_account'] );
  header( "Location: index.php" );
}
